...
def opencv():
    ...
    while True:
        ...
        frame_blend = cv2.line(frame_blend, (offset+xSet+30,330), ( 432+110 ,450), (255, 255, 255), 2)
        frame_blend = cv2.line(frame_blend, (offset+xSet-30,330), ( 432-110 ,450), (255, 255, 255), 2)
        ...
        # 調整大小為模型訓練時的大小 (30x30)
        resized_img = np.array(Image.fromarray(result_img_trapezoid_corrected).resize((30, 30)))
        ...
        roadClass = ['straight', 'turn left', 'turn right', 'none', 'T-intersection','crossRoad','right fork','left fork']
        predicted_class = roadClass[predicted_class]
        ...
        painter.drawText(20, 120, f"RoadAI: {predicted_class}")
        #painter.drawText(20, 150, f"Decision: {offset}")

        if tv > 0:
            tv = f"+{tv}"
        painter.drawText(20, 240, f"turnGain: {tv}")
        painter.drawText(20, 270, f"rightOffset: {int(sum(rightOffsetList)/len(rightOffsetList))}")
        painter.drawText(20, 300, f"leftOffset: {int(sum(leftOffsetList)/len(leftOffsetList))}")
